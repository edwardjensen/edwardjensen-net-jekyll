...
The `_plugins/rss_feed_generator.rb` plugin generates all RSS feeds from configuration in `_config.yml`. It replaces the previous approach of using Liquid templates and a separate featured tag feeds plugin.

### Configuration

Feeds are configured in the `rss_feeds:` section of `_config.yml`:
