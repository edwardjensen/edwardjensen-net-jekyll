...
All RSS feeds are generated by `_plugins/rss_feed_generator.rb` using configuration in `_config.yml`.

**Key files**:
- `_plugins/rss_feed_generator.rb` - Unified feed generator using Ruby `builder` gem
- `_config.yml` (`rss_feeds:` section) - Feed configuration

**Static feeds** (configured in `rss_feeds.feeds[]`):
...
**Dynamic feeds** (configured in `rss_feeds.dynamic_feeds[]`):
